...
-- TODO : try to generalize this lemma relaxing the hp `hf_one` and `hfderiv_one`, the statement should look something like: `fDiv f μ ν = ∫ x, (fDiv (statInfoFun 1 x) μ ν).toReal ∂(curvatureMeasure hf_cvx) + f 1 * ν univ + rightDeriv f 1 * (μ univ - ν univ)`. But it's not straightforward to modify the current proof, because if we make `f 1` and `rightDeriv f 1` appear inside the integral then we have to split the integral in 3 parts using `integral_add`, before changing it to a `LIntegral` (the new thing inside the integral is not nonnegative anymore, because `f 1` can be arbitrarily negative), but this needs us to prove the integrability of the first part (let's call it `A`), which is not trivial, and is part of the reason I decided to go through the LIntegral route. Maybe the way could be to do the cases where `A` is integrable and where it's not separately, or maybe the same but with `HasFiniteIntegral`, but I'm not sure if this is possible or easy to implement.
--this is the attempt to generalize the lemma:
...
--   classical
--   rw [fDiv_of_absolutelyContinuous h_ac, if_pos h_int]
--   simp_rw [fDiv_of_absolutelyContinuous h_ac, if_pos (integrable_statInfoFun_rnDeriv 1 _ _ _),
--     EReal.toReal_coe]
--   have (x : 𝒳) : f ((∂μ/∂ν) x).toReal
--       = ∫ y, statInfoFun 1 y ((∂μ/∂ν) x).toReal ∂(curvatureMeasure hf_cvx)
--         + f 1 + (rightDeriv f 1) * (((∂μ/∂ν) x).toReal - 1) := by
--     rw [integral_statInfoFun_curvatureMeasure hf_cvx hf_cont]
--     ring
--   simp_rw [this]
...
--     exact ((μ.measurable_rnDeriv ν).comp measurable_fst).ennreal_toReal
...
--     (fun a ↦ ∫ y, statInfoFun 1 y ((∂μ/∂ν) a).toReal ∂hf_cvx.curvatureMeasure) ν := by sorry --here is the major problem
--   rw [integral_add (integrable_add_const_iff.mpr h_int'), integral_add h_int' (integrable_const _), EReal.coe_add, EReal.coe_add]
--   swap; · exact Measure.integrable_toReal_rnDeriv.sub (integrable_const 1) |>.const_mul _
--   congr
...
--   · simp only [integral_const, smul_eq_mul, mul_comm, EReal.coe_mul]
--     congr
--     sorry --should be easy, it's just a coercion problem
--   · simp_rw [mul_sub, mul_one]
--     rw [integral_sub (Measure.integrable_toReal_rnDeriv.const_mul _) (integrable_const _),
--       integral_const, smul_eq_mul, mul_comm, integral_mul_left, ← mul_sub, EReal.coe_mul,
--       EReal.coe_sub, Measure.integral_toReal_rnDeriv h_ac]
--     congr
--     · sorry--should be easy, it's just a coercion problem, same as above
--     · sorry--should be easy, it's just a coercion problem, same as above
