...
See also https://math.ryerson.ca/~niushan/Stieltjes-integrals.pdf [4] for some facts about Riemann-Stieltjes and Lebesgue integrals.
...
Our integral is the same as the Lebesgue-Stieltjes integral defined in [4], 4.1 in [4] assures us that this integral corresponds to the Riemann-Stieltjes one in the case of monotone right-continuous functions and if the integral exists.
...
In our case both functions are Stieltjes functions, hence they are of bounded variation. However we also need to ask for one of the functions to be continuous, otherwise the Riemann-Stieltjes integral may not be true, for example if `f` and `α` have a common point of left discontinuity (see theorem 7.29 in [2]).
-/
