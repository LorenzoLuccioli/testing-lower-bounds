/-
Here I try to rewrite the Stieltjes.lean file from Mathlib, but for function on more general types, using hypotheses that are as minimal as possible
-/
...
/-- Bundled monotone right-continuous functions, used to construct Stieltjes measures. -/
structure StieltjesFunction (α β : Type*)
  [ConditionallyCompleteLinearOrder α] [ConditionallyCompleteLinearOrder β]
  [TopologicalSpace α] [TopologicalSpace β] where
  toFun : α → β
  mono' : Monotone toFun
  right_continuous' : ∀ x, ContinuousWithinAt toFun (Ici x) x
...
instance instCoeFun (α β : Type*) [ConditionallyCompleteLinearOrder α]
    [ConditionallyCompleteLinearOrder β] [TopologicalSpace α] [TopologicalSpace β] :
    CoeFun (StieltjesFunction α β) fun _ => α → β :=
  ⟨toFun⟩

initialize_simps_projections StieltjesFunction (toFun → apply)

variable {α β : Type*} [ConditionallyCompleteLinearOrder α] [ConditionallyCompleteLinearOrder β]
  [TopologicalSpace α] [TopologicalSpace β]

@[ext] lemma ext {f g : StieltjesFunction α β} (h : ∀ x, f x = g x) : f = g := by
  exact (StieltjesFunction.mk.injEq ..).mpr (funext h)

variable (f : StieltjesFunction α β)

theorem mono : Monotone f := f.mono'

theorem right_continuous (x : α) : ContinuousWithinAt f (Ici x) x :=
  f.right_continuous' x

theorem rightLim_eq [OrderTopology α] [OrderTopology β] (x : α) :
    Function.rightLim f x = f x := by
  rw [← f.mono.continuousWithinAt_Ioi_iff_rightLim_eq, continuousWithinAt_Ioi_iff_Ici]
  exact f.right_continuous' x

theorem iInf_Ioi_eq [DenselyOrdered α] [NoMaxOrder α] [OrderTopology α] [OrderTopology β] (x : α) :
    ⨅ r : Ioi x, f r = f x := by
  suffices Function.rightLim f x = ⨅ r : Ioi x, f r by rw [← this, f.rightLim_eq]
  rw [f.mono.rightLim_eq_sInf, sInf_image']
  rw [← neBot_iff]
  exact nhdsWithin_Ioi_self_neBot x

-- for this result I don't know how to write it, clearly it can be generalized at least to subsets of ℝ, but I don't know how to write it
-- theorem iInf_rat_gt_eq (f : StieltjesFunction α β) (x : α) :
...
@[simps]
protected def id (α : Type*) [ConditionallyCompleteLinearOrder α] [TopologicalSpace α] :
    StieltjesFunction α α where
  toFun := id
  mono' _ _ := id
  right_continuous' _ := continuousWithinAt_id

@[simp]
theorem id_leftLim [DenselyOrdered α] [NoMaxOrder α] [NoMinOrder α] [OrderTopology α] [T2Space α] (x : α) :
    leftLim (StieltjesFunction.id α) x = x :=
  tendsto_nhds_unique ((StieltjesFunction.id α).mono'.tendsto_leftLim x) <|
    continuousAt_id.tendsto.mono_left nhdsWithin_le_nhds

/-- Constant functions are Stieltjes function. -/
protected def const (c : β) : StieltjesFunction α β where
  toFun := fun _ ↦ c
  mono' _ _ := by simp
  right_continuous' _ := continuousWithinAt_const

@[simp] lemma const_apply (x : α) (c : β) : (StieltjesFunction.const c) x = c := rfl

instance instInhabited [hβ : Nonempty β] : Inhabited (StieltjesFunction α β) :=
  ⟨StieltjesFunction.const hβ.some⟩

/-- The sum of two Stieltjes functions is a Stieltjes function. -/
protected def add [Add β] [ContinuousAdd β]
    [CovariantClass β β (fun x1 x2 ↦ x1 + x2) fun x1 x2 ↦ x1 ≤ x2]
    [CovariantClass β β (swap fun x1 x2 ↦ x1 + x2) fun x1 x2 ↦ x1 ≤ x2]
    (f g : StieltjesFunction α β) : StieltjesFunction α β where
  toFun := fun x => f x + g x
  mono' := f.mono.add g.mono
  right_continuous' := fun x => (f.right_continuous x).add (g.right_continuous x)

instance [AddZeroClass β] [ContinuousAdd β]
    [CovariantClass β β (fun x1 x2 ↦ x1 + x2) fun x1 x2 ↦ x1 ≤ x2]
    [CovariantClass β β (swap fun x1 x2 ↦ x1 + x2) fun x1 x2 ↦ x1 ≤ x2] :
    AddZeroClass (StieltjesFunction α β) where
...
/-Mimmi
-spiegare cos'è la CAA
-spiega meglio che intendi per formato da usare, spendici un attimo in piu
-open source ≠ posso mettere altri documenti nell'app


-/

instance [AddCommMonoid β] [ContinuousAdd β]
    [CovariantClass β β (fun x1 x2 ↦ x1 + x2) fun x1 x2 ↦ x1 ≤ x2]
    [CovariantClass β β (swap fun x1 x2 ↦ x1 + x2) fun x1 x2 ↦ x1 ≤ x2] :
    AddCommMonoid (StieltjesFunction α β) where
  nsmul n f := nsmulRec n f
  add_assoc _ _ _ := ext fun _ ↦ add_assoc _ _ _
  add_comm _ _ := ext fun _ ↦ add_comm _ _
  __ := inferInstanceAs (AddZeroClass (StieltjesFunction α β))

-- try to generalize even more, generalize β? I don't know which class I could choose to have that if f is monotone then c • f is monotone
instance : Module ℝ≥0 (StieltjesFunction α ℝ) where
  smul c f := {
    toFun := fun x ↦ c * f x
...
@[simp] lemma add_apply (f g : StieltjesFunction α ℝ) (x : α) : (f + g) x = f x + g x := rfl
...
noncomputable def _root_.Monotone.stieltjesFunction [OrderTopology α] [NoMaxOrder α]
    [DenselyOrdered α] [OrderTopology β] [NoMaxOrder β] [NoMinOrder β]
    {f : α → β} (hf : Monotone f) :
    StieltjesFunction α β where
  toFun := rightLim f
  mono' x y hxy := hf.rightLim hxy
  right_continuous' := by
    intro x s hs
    obtain ⟨l, u, hlu, lus⟩ : ∃ l u : β, rightLim f x ∈ Ioo l u ∧ Ioo l u ⊆ s :=
      mem_nhds_iff_exists_Ioo_subset.1 hs
    obtain ⟨y, xy, h'y⟩ : ∃ (y : α), x < y ∧ Ioc x y ⊆ f ⁻¹' Ioo l u :=
    ...
    obtain ⟨a, za, ay⟩ : ∃ a : α, z < a ∧ a < y := exists_between hz.2
    calc
      rightLim f z ≤ f a := hf.rightLim_le za
      _ < u := (h'y ⟨hz.1.trans_lt za, ay.le⟩).2

theorem _root_.Monotone.stieltjesFunction_eq [OrderTopology α] [NoMaxOrder α]
    [DenselyOrdered α] [OrderTopology β] [NoMaxOrder β] [NoMinOrder β]
    {f : α → β} (hf : Monotone f) (x : α) :
    hf.stieltjesFunction x = rightLim f x :=
  rfl

theorem countable_leftLim_ne [OrderTopology α] [NoMinOrder α]
    [DenselyOrdered α] [OrderTopology β] [SecondCountableTopology β]
    (f : StieltjesFunction α β) : Set.Countable { x | leftLim f x ≠ f x } := by
...
variable (f : StieltjesFunction α ℝ)
/-- Length of an interval. This is the largest monotone function which correctly measures all
intervals. -/
def length (s : Set α) : ℝ≥0∞ :=
  ⨅ (a) (b) (_ : s ⊆ Ioc a b), ofReal (f b - f a)

@[simp]
theorem length_empty [hα : Nonempty α] : f.length ∅ = 0 :=
    nonpos_iff_eq_zero.1 <| iInf_le_of_le hα.some <| iInf_le_of_le hα.some <| by simp

@[simp]
theorem length_Ioc (a b : α) : f.length (Ioc a b) = ofReal (f b - f a) := by
  refine
...
theorem length_mono {s₁ s₂ : Set α} (h : s₁ ⊆ s₂) : f.length s₁ ≤ f.length s₂ :=
  iInf_mono fun _ => biInf_mono fun _ => h.trans

open MeasureTheory

/-- The Stieltjes outer measure associated to a Stieltjes function. -/
protected def outer : OuterMeasure α :=
  OuterMeasure.ofFunction (fun s ↦ f.length s) f.length_empty

theorem outer_le_length (s : Set α) : f.outer s ≤ f.length s :=
...
theorem length_subadditive_Icc_Ioo [CompactIccSpace α] [OrderClosedTopology α]
    {a b : α} {c d : ℕ → α} (ss : Icc a b ⊆ ⋃ i, Ioo (c i) (d i)) :
...
@[simp]
theorem outer_Ioc [OrderTopology α] [NoMaxOrder α] [NoMinOrder α] [DenselyOrdered α]
    (a b : α) : f.outer (Ioc a b) = ofReal (f b - f a) := by
...
theorem measurableSet_Ioi {c : α} : MeasurableSet[f.outer.caratheodory] (Ioi c) := by
...
theorem outer_trim [OrderTopology α] [NoMaxOrder α] [NoMinOrder α] [DenselyOrdered α]
    [MeasurableSpace α] [OpensMeasurableSpace α] : f.outer.trim = f.outer := by
...
theorem borel_le_measurable [OrderTopology α] [SecondCountableTopology α] :
    borel α ≤ f.outer.caratheodory := by
...
variable [OrderTopology α] [NoMaxOrder α] [NoMinOrder α] [DenselyOrdered α]
  [SecondCountableTopology α] [MeasurableSpace α] [BorelSpace α]

/-- The measure associated to a Stieltjes function, giving mass `f b - f a` to the
interval `(a, b]`. -/
protected irreducible_def measure : Measure α where
  toOuterMeasure := f.outer
  m_iUnion s hs := by
    refine f.outer.iUnion_eq_of_caratheodory fun i ↦ f.borel_le_measurable _ ?_
    convert (hs i)
    exact BorelSpace.measurable_eq.symm
  trim_le := f.outer_trim.le

@[simp]
theorem measure_Ioc (a b : α) : f.measure (Ioc a b) = ofReal (f b - f a) := by
  rw [StieltjesFunction.measure]
  exact f.outer_Ioc a b

@[simp]
theorem measure_singleton (a : α) : f.measure {a} = ofReal (f a - leftLim f a) := by
  obtain ⟨u, u_mono, u_lt_a, u_lim⟩ :
    ∃ u : ℕ → α, StrictMono u ∧ (∀ n : ℕ, u n < a) ∧ Tendsto u atTop (𝓝 a) :=
...
@[simp]
theorem measure_Icc (a b : α) : f.measure (Icc a b) = ofReal (f b - leftLim f a) := by
  ...
  · simp [hab, f.mono.le_leftLim hab]

@[simp]
theorem measure_Ioo {a b : α} : f.measure (Ioo a b) = ofReal (leftLim f b - f a) := by
...
@[simp]
theorem measure_Ico (a b : α) : f.measure (Ico a b) = ofReal (leftLim f b - leftLim f a) := by
  rcases le_or_lt b a with (hab | hab)
...
theorem measure_Iic {l : ℝ} (hf : Tendsto f atBot (𝓝 l)) (x : α) :
    f.measure (Iic x) = ofReal (f x - l) := by
  refine tendsto_nhds_unique (tendsto_measure_Ioc_atBot _ _) ?_
  simp_rw [measure_Ioc]
  exact ENNReal.tendsto_ofReal (hf.const_sub _)

-- I'm not sure how to prove this, do I need some additional hypothesis? I think it should be possible to prove it with these hps, though
-- theorem measure_Ici {l : ℝ} (hf : Tendsto f atTop (𝓝 l)) (x : α) :
...
--   have h_lt x : ∃ y : α, y < x := exists_lt x --does not work, I have to find another way, I need an element that is smaller than x, but that if x goes to top then y goes to top too

--   have h_le1 : ∀ x, f (h_lt x).choose ≤ leftLim f x :=
--     fun x => Monotone.le_leftLim f.mono (h_lt x).choose_spec
...
instance instIsLocallyFiniteMeasure : IsLocallyFiniteMeasure f.measure := by
  refine ⟨fun x ↦ ⟨Ioo (exists_lt x).choose (exists_gt x).choose, ?_, by simp⟩⟩
  exact Ioo_mem_nhds (exists_lt x).choose_spec (exists_gt x).choose_spec

lemma eq_of_measure_of_tendsto_atBot (g : StieltjesFunction α ℝ) {l : ℝ}
...
lemma eq_of_measure_of_eq (g : StieltjesFunction α ℝ) {y : α}
...
@[simp]
lemma measure_const (c : ℝ) : (StieltjesFunction.const (α := α) c).measure = 0 :=
  Measure.ext_of_Ioc _ _ (fun _ _ _ ↦ by simp)

@[simp]
lemma measure_add (f g : StieltjesFunction α ℝ) : (f + g).measure = f.measure + g.measure := by
...
@[simp]
lemma measure_smul (c : ℝ≥0) (f : StieltjesFunction α ℝ) : (c • f).measure = c • f.measure := by
